...
class Formatter {
    configure() {
        ...
        for (let name in stylus_supremacy_1.schema) {
            if (config.has(name)) {
    ...
    findStylintOptions(document, rootPath) {
        ...
        if (stylintPath) {
            try {
                ...
                return stylus_supremacy_1.createFormattingOptionsFromStylint(stylintOptions);
            }
            catch (ex) {
                console.error(ex);
            }
    ...
    format(document, documentOptions, cancellationToken, originalRange, ignoreErrors = false) {
        const rootDirx = vscode.workspace.getWorkspaceFolder(document.uri);
        const rootPath = rootDirx ? rootDirx.uri.fsPath : undefined;
        const formattingOptions = Object.assign(Object.assign(Object.assign({}, this.workspaceFormattingOptions), this.findStylintOptions(document, rootPath)), { tabStopChar: documentOptions.insertSpaces ? ' '.repeat(documentOptions.tabSize) : '\t', newLineChar: document.eol === vscode.EndOfLine.CRLF ? '\r\n' : '\n', wrapMode: !!originalRange // True for partial formatting, otherwise false
         });
        if (!originalRange && rootPath && stylus_supremacy_1.checkIfFilePathIsIgnored(document.fileName, rootPath, formattingOptions)) {
            return null;
        }
        try {
            let modifiedRange;
            ...
            const outputContent = stylus_supremacy_1.format(inputContent, formattingOptions);
            ...
            if (cancellationToken && cancellationToken.isCancellationRequested === true || outputContent.length === 0) {
                ...
                return [vscode.TextEdit.replace(modifiedRange, (insertNewLineAtFirst ? formattingOptions.newLineChar : '') +
                        outputContent +
